...
const Page1: React.FC = () => {
  ...
  // Aplica variáveis CSS da resolução ao carregar ou redimensionar
  useEffect(() => {
    function updateCSSVars() {
      const resolution = getCurrentResolution();
      applyResponsiveProperties(resolution);
    }
    updateCSSVars();
  ...
  useEffect(() => {
    ...
    const allLettersCorrected = state.audioURLs.every(
      (audioURL) => audioURL.isCorrect !== null
    );
  ...
  return (
    <div className=" flex h-screen w-screen max-w-full flex-col  overflow-hidden">
      ...
      <RetryMenu
        pageId={"pagina1"}
        isOpen={isRetryMenuOpen}
        setIsOpen={setIsRetryMenuOpen}
      />
      <div className="flex flex-1 flex-col">
        <HeaderMenu />
        <div className="flex flex-1 flex-container">
          <div className="record-card-container overflow-auto">
            <div className="card-content">
              ...
              />
              and listen to the letter in English, and repeat by recording your
              voice by clicking the button
              <img
                src={ButtonMic}
                alt="Button"
